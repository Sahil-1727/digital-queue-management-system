...
from models import ServiceCenter, Admin, User, SuperAdmin, ServiceCenterRegistration, Token
...
def init_db(app):
    with app.app_context():
        ...
        # Seed demo tokens (history + live queue) for all centers if none exist
        try:
            if Token.query.count() == 0:
                ...
                if users and centers:
                    ...
                    for center in centers:
                        ...
                        for i in range(4):
                            ...
                            all_tokens.append(Token(
                                ...
                                actual_service_start=start, actual_service_end=end,
                                completed_time=end,
                                estimated_service_start=start, estimated_service_end=end,
                                leave_time=start - timedelta(minutes=15), reach_time=start,
                                is_walkin=False,
                            ))

                        # 1 Online Serving
                        all_tokens.append(Token(
                            user_id=users[4 % len(users)].id,
                            service_center_id=center.id,
                            token_number='T005', status='Serving',
                            created_time=base_time,
                            ...
                            leave_time=serve_start - timedelta(minutes=15), reach_time=serve_start,
                            is_walkin=False,
                        ))

                        # 5 Active online queue
                        for i in range(5):
                            est_start = serve_start + timedelta(minutes=(i + 1) * avg)
                            all_tokens.append(Token(
                                user_id=users[(5 + i) % len(users)].id,
                                service_center_id=center.id,
                                ...
                                leave_time=est_start - timedelta(minutes=20), reach_time=est_start,
                                is_walkin=False,
                            ))
                        ...
                        all_tokens.append(Token(
                            ...
                            service_center_id=center.id,
                            token_number='W000', status='Serving',
                            created_time=base_time,
                    ...
                    db.session.add_all(all_tokens)
                    db.session.commit()
                    print(f"✅ Seeded {len(all_tokens)} demo tokens across {len(centers)} service centers")
        except Exception as e:
            print(f"⚠️ Error seeding demo tokens: {e}")
